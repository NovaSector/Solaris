...
/obj/item/gun/ballistic/arquebus
	...
	load_sound = 'modular_solaris/sound/foley/musketload.ogg'
	fire_sound = "modular_solaris/sound/arquebus/arquefire.ogg"
...
/obj/item/gun/ballistic/arquebus/shoot_live_shot(mob/living/user as mob|obj, pointblank = 0, mob/pbtarget = null, message = 1)
	fire_sound = pick("modular_solaris/sound/arquebus/arquefire.ogg", "modular_solaris/sound/arquebus/arquefire2.ogg", "modular_solaris/sound/arquebus/arquefire3.ogg",
				"modular_solaris/sound/arquebus/arquefire4.ogg", "modular_solaris/sound/arquebus/arquefire5.ogg")
	. = ..()
...
/datum/intent/shoot/arquebus/prewarning()
	if(mastermob)
		...
		playsound(mastermob, pick('modular_solaris/sound/foley/musketcock.ogg'), 100, FALSE)
...
/obj/item/gun/ballistic/arquebus/shoot_with_empty_chamber()
	playsound(src.loc, 'modular_solaris/sound/foley/musketcock.ogg', 100, FALSE)
	update_icon()
...
/obj/item/gun/ballistic/arquebus/attackby(obj/item/A, mob/user, params)
	...
	if(istype(A, /obj/item/ammo_box) || istype(A, /obj/item/ammo_casing))
		...
		if((loc == user) && (user.get_inactive_held_item() != src))
			return
		playsound(src, "modular_solaris/sound/arquebus/insert.ogg", 100, channel = gunchannel)
	...
	if(istype(A, /obj/item/ramrod))
		...
		if(!reloaded)
			if(chambered)
				...
				playsound(src, "modular_solaris/sound/arquebus/ramrod.ogg",  100, channel = gunchannel)
		...
		if(reloaded && !myrod)
			...
			myrod = R
			playsound(src, "modular_solaris/sound/foley/musketload.ogg", 100, channel = gunchannel)
...
/obj/item/gun/ballistic/arquebus/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	...
	for(var/obj/item/ammo_casing/CB in get_ammo_list(FALSE, TRUE))
		...
		BB.firer = user
	reloaded = FALSE
	user.mind.add_sleep_experience(/datum/skill/combat/firearms, (user.STAINT*2.125))
	..()
	var/fire_dir = user.dir
	var/turf/smoke_loc = get_ranged_target_turf(user, fire_dir, 1)
	new /obj/effect/particle_effect/sparks/muzzle(smoke_loc)
	spawn (5)
...
/obj/item/ammo_box/magazine/internal/arquebus
	...
	ammo_type = /obj/item/ammo_casing/caseless/rogue/bullet/solaris
	caliber = "musketball"
	max_ammo = 1
	start_empty = TRUE
...
/obj/item/gun/ballistic/arquebus_pistol/attackby(obj/item/A, mob/user, params)
	...
	if(istype(A, /obj/item/ammo_box) || istype(A, /obj/item/ammo_casing))
		...
		if((loc == user) && (user.get_inactive_held_item() != src))
			return
		playsound(src, "modular_solaris/sound/arquebus/insert.ogg",  100)
		user.visible_message("<span class='notice'>[user] forces \a [A] down the barrel of \the [src].</span>")
		..()
	...
	if(istype(A, /obj/item/ramrod))
		...
		if(!reloaded)
			if(chambered)
				user.visible_message("<span class='notice'>[user] begins ramming \the [R] down the barrel of \the [src] .</span>")
				playsound(src, "modular_solaris/sound/arquebus/ramrod.ogg",  100)
				if(do_after(user, load_time_skill, src))
					user.visible_message("<span class='notice'>[user] has finished reloading \the [src].</span>")
					reloaded = TRUE
				return
		if(reloaded && !myrod)
			user.transferItemToLoc(R, src)
			myrod = R
			playsound(src, "modular_solaris/sound/foley/musketload.ogg",  100)
			user.visible_message("<span class='notice'>[user] stows \the [R] under the barrel of \the [src].</span>")
		if(!chambered && !myrod)
			user.transferItemToLoc(R, src)
			myrod = R
...
/obj/item/gun/ballistic/arquebus_pistol/attack_self(mob/living/user)
	...
	var/pro_spin_sound = 'modular_solaris/sound/arquebus/gunspin.ogg'
	var/spin_sound
	if(firearm_skill <= 2)
...
/obj/item/ramrod
	name = "ramrod"
	icon = 'modular_solaris/icons/obj/items/arquebus_items.dmi'
